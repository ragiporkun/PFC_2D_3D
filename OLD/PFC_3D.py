...
def seed_grains_like_fipy(p: Params, device, dtype) -> Tuple[torch.Tensor, np.ndarray]:
    ...
    orientations = np.random.uniform(0, 2 * np.pi, size=centers.shape[0])
    ...
    for (xc, yc, zc), _alpha_ang in zip(centers, orientations):
        xShift = X - xc
        yShift = Y - yc
        zShift = Z - zc

        wave = 4.0 * B * (
            torch.cos(q * xShift / math.sqrt(2)) * torch.cos(q * yShift / math.sqrt(2))
            + torch.cos(q * yShift / math.sqrt(2)) * torch.cos(q * zShift / math.sqrt(2))
            + torch.cos(q * xShift / math.sqrt(2)) * torch.cos(q * zShift / math.sqrt(2))
        )

        mask = (xShift ** 2 + yShift ** 2 + zShift ** 2) <= (R ** 2)
        psi = torch.where(mask, wave, psi)

    grain_data = np.column_stack((centers, orientations))
...
# ============================================================
def main():
    os.makedirs("VTKSingle", exist_ok=True)
    ...
    p = Params(
        ...
        c=2*math.pi*math.sqrt(2),          # CHANGED: tighter beam (was 5.0) to localize heat
        psi0=0.0,
        T0=0.6,
        alpha=3.,
        beta=0.5,
        ...
        v_x=1.,
        power=7000.0,               # CHANGED: reduced power (was 2500) to limit peak temperature
        ...
        T_max=3.0,                   # NEW: cap temperature to stay in BCC stability window
    ...
    psi, grain_data = seed_grains_like_fipy(p, device, dtype)
    T = torch.full((p.nx, p.ny, p.nz), p.T0, device=device, dtype=dtype)

    np.savetxt("VTKSingle/grain_data.csv", grain_data, delimiter=",",
               header="x_center,y_center,z_center,orientation", comments="")
    np.save("VTKSingle/grain_data.npy", grain_data)
    ...
    for step in range(nsteps):
        if (step % vtk_every) == 0:
            vtk_filename = os.path.join("VTKSingle", f"fields_{step:06d}.vtk")
            write_vtk_structured_points(vtk_filename, p, {"psi": psi, "T": T})

        T_bc: Dict[str, Tuple[str, Optional[float]]] = {}

        if elapsed > 15.0:
            sink = 0.5                # CHANGED: stronger sink (was 1.2) to pull T back to T0 faster
            source_coeff = 1.0
