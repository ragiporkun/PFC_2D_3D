...
def main():
    os.makedirs("VTK_TimeScale", exist_ok=True)
    ...
    p = Params(
        ...
        beta=0.5,
        M=0.6,
        tau_psi=1.,                 # CHANGED: slower psi kinetics (was 0.6) to suppress stripe nucleation
        v_x=1.,
    ...
    np.savetxt("VTK_TimeScale/grain_data.csv", grain_data, delimiter=",",
               header="x_center,y_center,z_center,euler_a,euler_b,euler_c", comments="")
    np.save("VTK_TimeScale/grain_data.npy", grain_data)
    ...
    for step in range(nsteps):
        if (step % vtk_every) == 0:
            vtk_filename = os.path.join("VTK_TimeScale", f"fields_{step:06d}.vtk")
