...
os.makedirs("Plots_P1800_v1_alpha0.1_beta0.025_M0.6_1", exist_ok=True)
...
v_x    = 1.                         # scan speed (map to m/s in paper -- M5/R2-02)
power  = 1800.0
absorption = 0.3
...
N_STEPS = 1001
SAVE_TIMES = (1, 5, 10, 25, 50, 100, 250, 300, 500, 550, 750, 1000, 1250, 1500, 1750, 2000, 2500, 3000, 3500, 3750)
...
for step in range(N_STEPS):
    ...
    if step % 10 == 0:
        ...
        plt.tight_layout(); plt.savefig(f"Plots_P1800_v1_alpha0.1_beta0.025_M0.6_1/psiT_{step}.png"); plt.close()

    if int(round(elapsed)) in SAVE_TIMES:
        np.save(f"Plots_P1800_v1_alpha0.1_beta0.025_M0.6_1/psi_{int(round(elapsed))}.npy", psi.value)
        np.save(f"Plots_P1800_v1_alpha0.1_beta0.025_M0.6_1/T_{int(round(elapsed))}.npy", T.value)
