...
os.makedirs("Plots_P1800_v0.1_alpha3_beta0.5_M0.6_1.60", exist_ok=True)
...
psi_eq = (fp.TransientTerm(coeff=tau_psi, var=psi)
          == fp.DiffusionTerm(coeff=1/0.6, var=mu))
...
for step in range(N_STEPS):
    ...
    if step % 10 == 0:
        ...
        plt.tight_layout(); plt.savefig(f"Plots_P1800_v0.1_alpha3_beta0.5_M0.6_1.60/psiT_{step}.png"); plt.close()

    if int(round(elapsed)) in SAVE_TIMES:
        np.save(f"Plots_P1800_v0.1_alpha3_beta0.5_M0.6_1.60/psi_{int(round(elapsed))}.npy", psi.value)
        np.save(f"Plots_P1800_v0.1_alpha3_beta0.5_M0.6_1.60/T_{int(round(elapsed))}.npy", T.value)
