...
M    = 0.06                            # thermal conductivity (diffusion coeff in T_eq)
tau_psi = 1.0
...
ALPHA_CP   = 3.0    # heat capacity c_p. rev8 used 0.1 (~30x too small). USE ~3.
...
v_x    = 1.0                           # scan speed (map to m/s in paper -- M5/R2-02)
power  = 1500.0
absorption = 0.3
...
N_STEPS = 2501
SAVE_TIMES = (1, 5, 10, 25, 50, 1000, 1300, 1800, 2000, 2500)
...
T_eq = (fp.TransientTerm(coeff=ALPHA_CP, var=T)
        ...
        + fp.TransientTerm(coeff=(BETA_LATENT + psi), var=psi)     # (beta+psi) dpsi/dt
...
# ======================================================================
if MODE == "full":
    # polycrystalline triangular seeds (unchanged from rev8)
    initial_psi = psi0 * np.ones_like(x.value)
    xc = [40,50,60,160,140,150,240,250,245,335,350,360,440,460,450,520,570,550,630,660,650,760,750,780]
    yc = [35,110,160,40,100,165,30,115,170,40,90,160,25,100,175,50,110,155,40,108,175,40,100,150]
    ang= [-0.9,0.4,-0.2,0,-0.6,0.3,-0.8,0.9,-0.5,0.1,-0.1,0.75,0.95,0.5,-0.1,0.05,-0.85,0.48,0.75,-0.1,-0.7,0.05,0.82,-0.6]
    A, pre, p = psi0, -0.5, 4 * np.pi / np.sqrt(3)
    B = ((4 - 12*A) + 4*np.sqrt(-15*(1 - 1/T0) + 24*A + 1 - 36*A**2)) / 15
    for (cx, cy), a in zip(zip(xc, yc), ang):
        xs, ys = x.value - cx, y.value - cy
        wave = A + B*(np.cos(2*np.pi*(xs*np.cos(a)+ys*np.sin(a))/p) *
                      np.cos(2*np.pi*(xs*np.sin(a)-ys*np.cos(a))/(np.sqrt(3)*p)) -
                      pre*np.cos(4*np.pi*(xs*np.sin(a)-ys*np.cos(a))/(np.sqrt(3)*p)))
        m = (xs**2 + ys**2) <= 12.0**2
        initial_psi[m] = wave[m]
    psi.setValue(initial_psi)
else:
...
for step in range(N_STEPS):
    ...
    if step % 50 == 0:
        ...
        axT = axes[-1]
        imT = axT.imshow(T.value.reshape(ny, nx), cmap='rainbow', origin='lower',
                         vmin=0, vmax=1.2, extent=[0, Lx, 0, Ly])  # physical scale ~T_m
        axT.contour(T.value.reshape(ny, nx), levels=[1.0], colors='black',
                    origin='lower', extent=[0, Lx, 0, Ly])
        axT.set_ylabel('T'); fig.colorbar(imT, ax=axT)
        plt.tight_layout(); plt.savefig(f"Plots/psiT_{step}.png"); plt.close()

    if int(round(elapsed)) in SAVE_TIMES:
        np.save(f"Plots/psi_{int(round(elapsed))}.npy", psi.value)
        np.save(f"Plots/T_{int(round(elapsed))}.npy", T.value)
